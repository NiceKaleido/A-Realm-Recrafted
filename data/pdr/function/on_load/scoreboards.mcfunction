scoreboard objectives add pdr.technical dummy
scoreboard objectives add pdr.respawned deathCount
scoreboard objectives add pdr.health dummy
scoreboard objectives add pdr.deathcounter dummy
scoreboard objectives add pdr.lost.health.counter dummy
scoreboard objectives add pdr.days.no.death dummy
scoreboard objectives add pdr.days.no.death.reset dummy
scoreboard objectives add pdr.daily.deathcounter deathCount

##modify scoreboards
scoreboard objectives setdisplay list pdr.days.no.death